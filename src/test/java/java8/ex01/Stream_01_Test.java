...
public class Stream_01_Test {
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        List<Order> result = orders.stream().filter(t -> t.getCustomer().getFirstname().equals("Johnny")).filter(t -> t.getPizzas().stream().anyMatch(a -> a.getPrice() >= 1300)).collect(Collectors.toList());
    ...
    public void test_stream_findFirst() throws Exception {
        ...
        Optional<Order> result = orders.stream().findFirst().filter(t -> t.getCustomer().getFirstname().equals("Sophie"));
